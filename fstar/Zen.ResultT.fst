...
val ofOption(#a:Type): string -> option a -> a `resultT` 0
let ofOption(#_) msg = function
  | Some v -> ok v
  | None -> failw msg

val ofOptionT(#a:Type)(#n:nat): string -> a `OptT.t` n -> a `resultT` n
let ofOptionT #_ #_ msg mx = mx `Cost.bind` (ofOption msg)
