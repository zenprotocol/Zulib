...
let show (x : uint64) : Prims.string = fsharpStringConvert (x.ToString())

let showPad (x : uint64) : Prims.string = fsharpStringConvert (x.ToString("D20"))

let read (s : Prims.string) : uint64 = Checked.uint64 (fstarStringConvert s)
